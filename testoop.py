...
class Shop:
    ...
    def process_order(self, customer):
        total_order_cost = sum(product.quantity * product.price for product in customer.products)

        if total_order_cost > customer.cash:
            raise ValueError(f"Not enough cash to complete the order. Order cost: {total_order_cost}, Available cash: {customer.cash}")
